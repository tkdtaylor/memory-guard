...
| 005 | Publish / remote follow-up (git remote + push) | `005-publish-remote-followup-test-spec.md` | TC-001…TC-003 (planned) | ⚠️ | **SKIPPED-blocked (operator decision required).** REQ-001 gates execution on the operator confirming owner + **public vs. private** visibility — an irreversible publish step `/autopilot` (autonomous) will not make unilaterally. Stays in `docs/tasks/backlog/`. The one invariant it owns that is enforceable locally is met now: **all 10 first-party `.go` files carry the `// SPDX-License-Identifier: Apache-2.0` header** (incl. the 4 added by tasks 001–004: `detector_native_test.go`, `detector_corpus_test.go`, `residue.go`, `residue_test.go`, `poisoning_suite_test.go`), `go build ./...` clean. Remaining work (create remote, push, confirm DCO wiring, mark TODO.md done) resumes once the operator supplies owner + visibility. |
