...
|---------|---------|-----------|---------------|--------|-------------|
| 001 | Resolve the `Detector` backend (memory-guard tracer + ADR) | `001-detector-backend-tracer-test-spec.md` | TC-001…TC-006 | 🟡 | L6: `go run . write` with `NativeDetector` (ADR-002, Go-native in-process) → PII `pii:EMAIL`-redacted, injection `allow:false`/`stored_id:null`, benign clean. L2: `go test -count=1 ./...` → `ok …memory-guard 0.290s`. Detection cost ~5.6 µs/op (budget <1 ms). TC-004 (dep-scan): no new dep → trivially clear. `guard.go`/`ipc.go`/contract untouched. Awaiting spec-verifier APPROVE before ✅. |
