...
| 004 | PII recognizer coverage hardening (behind the Detector seam) | `004-pii-recognizer-coverage-test-spec.md` | TC-001…TC-005 | 🟡 | L5: `go test -v -run TestCorpusSummary ./...` → 9 categories, recall=1.00 per category, precision=1.00 (0 FP / 9 hard negatives) for both RegexDetector and NativeDetector. L2: `go test -count=1 ./...` → `ok github.com/tkdtaylor/memory-guard 0.578s`. guard.go/ipc.go/main.go/CONTRACT.md untouched. |
