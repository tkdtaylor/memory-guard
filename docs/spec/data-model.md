...
- **`MemoryStore`** (`store.go`): the storage seam — the storage analogue of the `Detector` seam. The
  ...
  type MemoryStore interface {
      ...
      All() []entry                  // every survivor, non-nil; the residue scan iterates this
  }
  ```
...
{ "confirmed": true, "residue_detected": true,
  "residue_summary": "normalized residue of deleted content survives in entry \"note: John's balance is $5k…\": \"5000\"",
  "deletion_hash": "64daabe2…bedf49" }
```
...
- `residue_detected` is the result of a guard-side scan (ADR-003) of the **remaining** store —
  the store's `All()` survivors across **every** backing index/copy (ADR-005) — for a verbatim or
  near-verbatim fragment of the just-deleted content (a tiered normalized substring /
  contiguous-phrase / token-overlap match, stdlib-only — **not** a `Detector` concern). Because the
  scan runs over the survivors (after the target is removed from every index), a deleted entry can
  never flag itself.
- `residue_summary` (string) is present **only** when `residue_detected:true`; it names the match
  class (`verbatim` / `normalized` / `phrase` / `token-overlap N%`) and the surviving entry —
  referenced by a short **content snippet** (the residue scan operates over `All()` survivors, which
  carry no map id) — plus the matched fragment. Callers that ignored the new fields are unaffected
  (additive).
