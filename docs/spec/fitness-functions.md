...
| F-006 | Write-gate adversarial recall ≥ threshold | security | `TestPoisoningRecallPrecision` runs 32 labelled adversarial cases (MINJA / GRAGPoison / context-window injection) through `ValidateWrite` and asserts recall ≥ 0.55 and precision ≥ 0.55 per registered backend | recall ≥ 0.55, precision ≥ 0.55 (RegexDetector/NativeDetector v0 baseline) | `go test -run TestPoisoning ./...` | block | active | Turns "passes its own examples" into a measured adversarial bar (task 002). The v0 baseline is honest: recall=0.69, precision=0.85 measured on 2026-06-19 against 32 poisoning / 14 benign; 10 miss classes documented (see `poisoning_suite_test.go` corpus notes). A future stronger backend raises the threshold constants in `backendThresholds` without touching the corpus. |
