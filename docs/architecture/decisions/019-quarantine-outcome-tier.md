...
A mandatory security-auditor pass on the write-gate diff (`guard.go`, `detector.go`, the `RegexDetector` borderline pattern) runs after the task returns, before promotion to ✅. The mechanical zero-false-positive sweep across all four benign/hard-negative corpora (section 3, `TestTC002DetectBorderlineNarrow`) is the evidence submitted for that audit.

**Verdict: APPROVE for merge (2026-07-14), zero blocking findings.** The auditor independently confirmed: the fail-closed injection gate is byte-for-byte unchanged and takes priority over quarantine (block early-returns before the quarantine routing); the measured `poisoningRecallFloor` (0.80) / `poisoningPrecisionFloor` (0.85) are unchanged; quarantine content never reaches a normal `validate_read` (the exclusion post-filter is load-bearing, confirmed by an independently reproduced mutation probe); `review_quarantine` is redacted, read-only, and quarantine-only (`found:false` for both unknown and non-quarantined ids, so it is not a scope-bypassing id-lookup oracle); `allow == (state != "block")` holds on every path including task 021's reserved-key rejects; and the `quarantined` bit round-trips durably through `FileStore` (non-`omitempty`, so a restart cannot silently un-quarantine).

**Advisory SEC-001 (Medium, non-blocking, accepted).** The v0 placeholder pattern `(?i)pretend\s+(?:the\s+above|this)\s+never\s+happened` also matches ordinary benign retractions ("let's just pretend this never happened"), so a legitimate write carrying that phrase is quarantined and hidden from reads (a denial-of-memory). It is accepted rather than blocking because: it passes the REQ-009 fixture gate (zero collision on every existing benign / hard-negative corpus, the tokens appear only in this task's positive controls); a false-positive routes to **quarantine, not block**, so it is isolated-and-recoverable via `review_quarantine`, not lost; and this ADR already frames `DetectBorderline` as a conservative placeholder whose real quarantine-vs-allow decision authority is deferred to the policy-engine. Tracked as tuning input for the future classifier: tighten the trigger to require an adjacent context-reset cue (imperative + "start fresh" / "forget" / "new session"), or record the residual benign-collision rate as a policy-engine input. Not a merge blocker.
